...
def login(request):
    if request.method == 'POST':
        ...
        email_id= User_Registration.objects.get(email=email)
        try:
    ...
    if request.method == 'POST':
        ...
        for pro in product:
            order_obj = Order(
                address= address,
                mobile= mobile,
                customer= User_Registration(id=customer_id),
...
class User_RegistrationViewSet(viewsets.ModelViewSet):
